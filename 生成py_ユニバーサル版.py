...
# 条件付きファイル
nijisousaku = os.path.join(BASE_DIR, "二次創作.txt")
choukasoku  = os.path.join(BASE_DIR, "超加速.txt")
tanohito    = os.path.join(BASE_DIR, "他の人.txt")
...
# OUTPUT_DIR は上部で定義済み (os.path.join(BASE_DIR, "output"))
...
if final_result is None:
    ...
    # クリップボードへコピー（Windows以外でもクラッシュしないクロスプラットフォーム対応）
    try:
        if os.name == "nt":
    ...
    if "--stdout" in sys.argv or "-p" in sys.argv:
        ...
        print("規定の形式（AA＋台詞中心）で【SS本編の本文のみ】を執筆してこのチャットに出力せよ。資料サンプル台詞・Markdown見出し・仕様説明Q&Aを本文に出すな。")
        print("=" * 60)
